...
export class RelationParsingService extends OBC.Component implements OBC.Disposable {
  ...
  constructor(components: OBC.Components) {
    ...
    this._openingsSceneGroup.name = "TRA88_IfcOpenings_Group";
    this._openingsSceneGroup.visible = false;

    this._spatialZonesSceneGroup.name = "TRA88_IfcSpatialZones_Group";
    this._spatialZonesSceneGroup.visible = false;

    // 모델이 삭제되거나 닫힐 때 캐시 자동 정리
  ...
  async getModelRelations(model: FRAGS.FragmentsModel | any): Promise<ModelRelationData> {
    ...
    try {
      ...
      this._modelRelationsCache.set(modelKey, emptyData);
      return emptyData;
    }
  }
  ...
  private async _getIfcBuffer(model: any): Promise<{ name: string; content: Uint8Array } | null> {
    ...
    const dbId =
      (model as any).dbId ||
      sharedIFC.getIfcIdByModelUUID((model as any).uuid) ||
      sharedIFC.getIfcIdByModelUUID((model as any).modelId);
    ...
    if (!ifcData) {
      ...
      const matched = sharedIFC.list.find(
        (f) => f.name === (model as any).name || (model as any).name?.includes(f.name)
      );
      if (matched) {
  ...
  async highlightElements(modelIdMap: OBC.ModelIdMap, autoShow: boolean = true) {
    ...
    if (autoShow) {
      if (hasOpenings && !this.isOpeningsVisible) {
        await this.showOpenings();
      }
      if (hasZones && !this.isSpatialZonesVisible) {
        await this.showSpatialZones();
      }
    }
  }
  ...
  applySelectionHighlight(styleName: string, modelIdMap: OBC.ModelIdMap, styleDef?: any) {
    ...
    if (styleName === "select") {
      this.clearSelectionHighlight("select");
    }
  ...
  getParentForOpening(modelKey: string, openingExpressId: number): number | null {
    ...
    return relData.openingToParent.get(openingExpressId) ?? null;
  }

  getFillingsForOpening(modelKey: string, openingExpressId: number): number[] {
    const relData = this._modelRelationsCache.get(modelKey);
    if (!relData) return [];
    return relData.openingToFillings.get(openingExpressId) || [];
  }

  getOpeningForFilling(modelKey: string, fillingExpressId: number): number | null {
    const relData = this._modelRelationsCache.get(modelKey);
    if (!relData) return null;
    return relData.fillingToOpening.get(fillingExpressId) ?? null;
  }

  getSpatialZones(modelKey: string): IfcSpatialZoneData[] {
    const relData = this._modelRelationsCache.get(modelKey);
    if (!relData) return [];
    return Array.from(relData.spatialZones.values());
  }
  ...
  getReferencedElementsForZone(modelKey: string, zoneExpressId: number): number[] {
    const relData = this._modelRelationsCache.get(modelKey);
    if (!relData) return [];
    return relData.zoneToElements.get(zoneExpressId) || [];
  }
}
