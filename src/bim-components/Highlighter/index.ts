  ...
  private onMouseUp = async (event: MouseEvent) => {
    ...
    if (autoHighlightOnClick && selectEnabled) {
      ...
      if (!result || result.localId === undefined || result.localId === null) {
        ...
        // If a modifier key is pressed, do nothing when clicking on empty space
        return;
      }
      ...
      // [Ctrl]+[click]: Add to selection. If already selected, do nothing.
      if (event.ctrlKey) {
        if (!isSelected) {
      ...
      // [Shift]+[click]: Remove from selection. If not selected, do nothing.
      if (event.shiftKey) {
        if (isSelected) {
          await this.clear(selectName, found);
        }
        return;
      }
